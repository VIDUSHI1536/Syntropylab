      ...
      >
        ...
        <CardContent className="p-4 pt-2 space-y-3">
          <div className="space-y-1">
            ...
            <div
              className={cn(
                "text-sm p-2 rounded-md max-h-[100px] overflow-y-auto",
          ...
          <div className="space-y-1">
            ...
            <div
              className={cn(
                "text-sm p-2 rounded-md max-h-[80px] overflow-y-auto",
                isDark ? 'bg-white/5' : 'bg-muted/30'
              )}
          ...
          <div className="space-y-1">
            ...
            <div className={cn(
              "text-sm p-2 rounded-md max-h-[150px] overflow-y-auto border",
              isDark ? 'bg-blue-500/10 border-blue-500/20' : 'bg-blue-50/50 border-blue-100'
            )}>
              {row.output || <span className={cn(
                "italic",
                isDark ? 'text-white/40' : 'text-muted-foreground'
              )}>Not generated</span>}
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            {evaluators.map(e => (
              <div key={e._id} className={cn(
                "flex flex-col p-2 rounded border",
                isDark ? 'bg-purple-500/10 border-purple-500/20' : 'bg-purple-50/50 border-purple-100'
              )}>
                <span className={cn(
                  "text-[10px] uppercase truncate",
                  isDark ? 'text-white/60' : 'text-muted-foreground'
                )}>{e.name}</span>
                <div className="mt-1">{getScoreBadge(row[`eval_${e._id}`]) || <span className={cn(
                  "text-xs",
                  isDark ? 'text-white/40' : 'text-muted-foreground'
                )}>-</span>}</div>
              </div>
            ...
            <div className={cn(
              ...
              <span className={cn(
                "text-xs font-mono",
                isDark ? 'text-white' : 'text-foreground'
              )}>{row.latency ? `${row.latency.toFixed(2)}s` : '-'}</span>
            </div>
            <div className={cn(
              "flex flex-col p-2 rounded",
              isDark ? 'bg-white/5' : 'bg-muted/30'
          ...
          <div className={cn(
            ...
            <div className="flex items-center gap-1">
              <Button
                ...
                className={cn(
                  "h-7 w-7",
                  row.humanEvaluation === 'up' ? 'bg-green-500 hover:bg-green-600' : '',
                  isDark && row.humanEvaluation !== 'up' && 'border-white/10 hover:bg-white/10'
                )}
              ...
              <Button
                ...
                className={cn(
                  "h-7 w-7",
                  row.humanEvaluation === 'down' ? 'bg-red-500 hover:bg-red-600' : '',
                  isDark && row.humanEvaluation !== 'down' && 'border-white/10 hover:bg-white/10'
                )}
  ...
  return (
    <TooltipProvider>
      <div className={cn(
        ...
        {/* Header Breadcrumb */}
        <div className={cn(
          "flex items-center justify-between py-4 px-6 border-b shrink-0",
          ...
          <Card className={cn(
            ...
            <CardContent className="p-4 pt-0">
              <div className={cn(
                "text-2xl font-bold",
                isDark ? 'text-white' : 'text-foreground'
              )}>{metrics.humanPassRate}%</div>
              <p className={cn(
                "text-xs",
                isDark ? 'text-white/50' : 'text-muted-foreground'
              )}>Based on manual reviews</p>
            </CardContent>
          </Card>
          <Card className={cn(
            "rounded-2xl border transition-colors",
        ...
        {/* Toolbar */}
        <div className={cn(
          "px-4 md:px-6 py-3 border-b flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0",
          isDark ? 'border-white/10 bg-[#121022]' : 'border-border bg-background'
        )}>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select value={selectedDatasetId || 'none'} onValueChange={(v) => setSelectedDatasetId(v === 'none' ? null : v)}>
              <SelectTrigger className={cn(
                "w-full md:w-[180px] h-8 rounded-xl",
...
>
  ...
  Open Playground
              ...
              <DialogContent className={cn(
                ...
                <DialogHeader>
                  <DialogTitle className={cn(isDark && 'text-white')}>Upload Dataset</DialogTitle>
                  <DialogDescription className={cn(isDark && 'text-white/60')}>
                    Upload a CSV file with your evaluation data.
                  </DialogDescription>
                </DialogHeader>
            ...
            <Select value={selectedModel} onValueChange={setSelectedModel}>
              <SelectTrigger className={cn(
                "w-full md:w-[200px] h-8 rounded-xl",
                isDark && 'bg-white/5 border-white/10 text-white'
              )}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Dialog open={newEvaluatorOpen} onOpenChange={(open) => {
              ...
              <DialogContent className={cn(
                ...
                <DialogHeader>
                  <DialogTitle className={cn(isDark && 'text-white')}>Add Evaluator</DialogTitle>
                  <DialogDescription className={cn(isDark && 'text-white/60')}>
                    Choose from evaluation gallery or create your own custom evaluator.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 pt-4">
                  <Tabs value={newEvaluatorType} onValueChange={(v) => setNewEvaluatorType(v as 'gallery' | 'custom')}>
                        ...
                        <>
                          <div className="grid grid-cols-2 gap-3">
                            {evaluatorGallery.map(evaluator => (
                              <Card
                                key={evaluator._id}
                                className={cn(
                                  "cursor-pointer transition-all hover:border-primary rounded-xl",
                                  selectedGalleryEvaluator === evaluator._id ? 'border-primary ring-2 ring-primary' : '',
                                  isDark ? 'bg-white/5 border-white/10 hover:bg-white/10' : ''
                                )}
                          ...
                          {selectedGalleryEvaluator && (
                            <div className={cn(
                              ...
                              isDark ? 'bg-white/5 border-white/10' : 'bg-muted/50'
                            )}>
                              <Label className={cn(
                                "text-sm font-semibold mb-2 block",
                                isDark && 'text-white'
                              )}>Evaluation Prompt Preview</Label>
                              <div className={cn(
                    ...
                    <TabsContent value="custom" className="space-y-4">
                      <div>
                        <Label className={cn(isDark && 'text-white')}>Evaluator Name</Label>
                        <Input
                      ...
                      <div>
                        <Label className={cn(isDark && 'text-white')}>Custom Evaluation Prompt</Label>
                        <Textarea
        ...
        <div className="flex-1 overflow-auto">
          <table className={cn(
            "w-full text-sm border-collapse min-w-max",
            isDark && 'text-white'
          )}>
            <thead className={cn(
              "sticky top-0 z-10",
              isDark ? 'bg-[#1a1625]' : 'bg-muted/50'
            )}>
              <tr>
                <th className={cn(
                  "p-2 text-left font-medium border-b w-8",
                  isDark ? 'border-white/10' : 'border-border'
                )}></th>
                {visibleColumns.includes('input') && (
                  <th className={cn(
                    "p-2 text-left font-medium border-b min-w-[200px]",
                    isDark ? 'border-white/10 text-white/70' : 'border-border'
                  )}>INPUT</th>
                )}
                ...
                {visibleColumns.includes('output') && (
                  <th className={cn(
                    "p-2 text-left font-medium border-b min-w-[250px]",
                    isDark ? 'border-white/10 text-white/70 bg-blue-500/10' : 'border-border bg-blue-50'
                  )}>OUTPUT</th>
                )}
                {visibleColumns.includes('modelNickname') && (
                  <th className={cn(
                    "p-2 text-left font-medium border-b min-w-[150px]",
                    isDark ? 'border-white/10 text-white/70' : 'border-border'
                  )}>MODEL</th>
                )}
                ...
                {evaluators.map(e => visibleColumns.includes(`eval_${e._id}`) && (
                  <th key={e._id} className={cn(
                    "p-2 text-center font-medium border-b min-w-[100px]",
                    isDark ? 'border-white/10 text-white/70 bg-purple-500/10' : 'border-border bg-purple-50'
                  )}>
                    {e.name.toUpperCase()}
                  </th>
                ))}
                {visibleColumns.includes('outputTokens') && (
                  <th className={cn(
                    "p-2 text-right font-medium border-b min-w-[80px]",
                    isDark ? 'border-white/10 text-white/70' : 'border-border'
                  )}>OUT TOKENS</th>
                )}
                ...
                <th className={cn(
                  "p-2 text-center font-medium border-b w-10",
                  isDark ? 'border-white/10' : 'border-border'
                )}>
                  <MoreHorizontal className={cn(
                    "w-4 h-4 mx-auto",
            ...
            <tbody>
              {rows.map((row, idx) => (
                <tr key={row.id} className={cn(
                  ...
                  isDark ? 'border-white/10 hover:bg-white/5' : 'border-border hover:bg-muted/30'
                )}>
                  ...
                  {visibleColumns.includes('input') && (
                    <td className="p-2">
                      {editingCell?.rowId === row.id && editingCell?.column === 'input' ? (
                        ...
                        <div
                          className={cn(
                            "cursor-text p-1 rounded max-w-[300px] truncate transition-colors",
                  ...
                  {visibleColumns.includes('systemInstructions') && (
                    <td className="p-2">
                      {editingCell?.rowId === row.id && editingCell?.column === 'systemInstructions' ? (
                        ...
                        <div
                          className={cn(
                            "cursor-text p-1 rounded max-w-[250px] truncate transition-colors",
                            isDark ? 'text-white/60 hover:bg-white/10' : 'text-muted-foreground hover:bg-muted/50'
                          )}
        ...
        {/* Footer */}
        <div className={cn(
          "px-6 py-2 border-t flex items-center justify-between text-sm shrink-0",
          isDark ? 'border-white/10 text-white/60 bg-[#121022]' : 'border-border text-muted-foreground bg-background'
        )}>
        ...
        <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
          <DialogContent className={cn(
            ...
            <DialogHeader>
              <DialogTitle className={cn(isDark && 'text-white')}>Settings & API Keys</DialogTitle>
              <DialogDescription className={cn(isDark && 'text-white/60')}>
